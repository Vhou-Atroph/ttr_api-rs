mod tests {
    ...
    fn is_cog_not_invading() {
        let inv = Invasions::Invasion::new(ttr_api::makeclient()).unwrap();
        assert_eq!(inv.cog_invading("Director of Ambush Marketing"),false)
    }
    #[test]
    fn correct_fo_zone() {
        let walway = 3100;
    ...
    fn real_doodle_render() {
        ...
        assert_eq!(doodle.render(256,"png"),String::from("rendition.toontownrewritten.com/render/64ffff02ff0111030401/doodle/256x256.png")) //Needs to be a real doodle in Blam Canyon, Toontown Central on the day it's tested.
    }
}